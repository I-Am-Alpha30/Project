...
class _AddProductState extends State<AddProduct> {
  ...
  GroupTaxModel? selectedGroupTaxModel;
  String selectedTaxType = 'Inclusive'; // Default value can be 'Inclusive' or 'Exclusive'
  double totalAmount = 0.0; // Define the totalAmount variable
  bool checkProductName({required String name, required String id}) {
    for (var element in widget.warehouseBasedProductModel) {
      print('name: ${element.productName}, id: ${element.productID}');
      if (element.productName.toLowerCase() == name.toLowerCase() && element.productID == id) {
...
@override
  ...
  double calculateAmountFromPercentage(double percentage, double amount) {
    return (percentage / 100) * amount;
  }

  // Adjust sales prices based on tax type
  void adjustSalesPrices() {
    double margin = double.tryParse(marginController.text) ?? 0;
    double purchasePrice = double.tryParse(productPurchasePriceController.text) ?? 0;
    double salesPrice = 0;
    double excPrice = 0;
    double taxRate = selectedGroupTaxModel?.taxRate.toDouble() ?? 0;
    double taxAmount = calculateAmountFromPercentage(taxRate, purchasePrice);

    if (selectedTaxType == 'Inclusive') {
      // For inclusive tax, the sales price includes tax
      salesPrice = purchasePrice + calculateAmountFromPercentage(margin, purchasePrice);
      productSalePrice = salesPrice.toStringAsFixed(2);
      productDealerPrice = salesPrice.toStringAsFixed(2);
      productWholeSalePrice = salesPrice.toStringAsFixed(2);
      incTaxAmount = purchasePrice.toStringAsFixed(2);
      excTaxAmount = (salesPrice - taxAmount).toStringAsFixed(2);
    } else {
      // For exclusive tax, the sales price is calculated separately from tax
      salesPrice = purchasePrice + calculateAmountFromPercentage(margin, purchasePrice) + taxAmount;
      excPrice = purchasePrice + taxAmount;
      productSalePrice = salesPrice.toStringAsFixed(2);
      productDealerPrice = salesPrice.toStringAsFixed(2);
      productWholeSalePrice = salesPrice.toStringAsFixed(2);
      incTaxAmount = purchasePrice.toStringAsFixed(2);
      excTaxAmount = excPrice.toStringAsFixed(2);
    }

    setState(() {
      // Update the state with the new values
      incTaxController.text = incTaxAmount;
      excTaxController.text = excTaxAmount;
    });
  }
                                                  ...
                                                  children: [
                                                    Expanded(
                                                      child: TextFormField(
                                                        validator: (value) {
                                                          if (productDealerPrice.isEmptyOrNull) {
                                                            return 'Product Dealer Price is required.';
                                                          } else if (double.tryParse(productDealerPrice) == null) {
                                                            return 'Enter price in number.';
                                                          } else {
                                                            return null;
                                                    ...
                                                    Expanded(
                                                      child: TextFormField(
                                                        validator: (value) {
                                                          if (productWholeSalePrice.isEmptyOrNull) {
                                                            return 'Product Wholesale Price is required.';
                                                          } else if (double.tryParse(productWholeSalePrice) == null) {
                                                            return 'Enter price in number.';
                                                          } else {
                                                            return null;
  ...
  DropdownButton<String> getTaxType() {
    List<String> taxTypeList = ['Inclusive', 'Exclusive'];
    return DropdownButton<String>(
      hint: const Text('Select Tax Type'),
      onChanged: (String? value) {
        setState(() {
          selectedTaxType = value!;
          adjustSalesPrices(); // Update sales prices when tax type changes
        });
      },
      value: selectedTaxType,
      items: taxTypeList.map((String items) {
        return DropdownMenuItem(
          value: items,
          child: Text(items),
        );
  ...
  bool isSerialNumberUnique({required List<String> allList, required String newSerial}) {
    for (var element in allList) {
      if (element.toLowerCase() == newSerial.toLowerCase()) {
        return false;
      }
    }
    return true;
